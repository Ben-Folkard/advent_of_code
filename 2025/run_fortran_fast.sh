...
if [ -z "$optional_libraries" ]; then
    ...
    gfortran -o "$output_name" -g -Wall -O3 -fcheck=all -fmax-errors=1 -std=f2008 "$source_file"
else
    # Compile with additional libraries
    gfortran -o "$output_name" -g -Wall -O3 -fcheck=all -fmax-errors=1 -std=f2008 "$source_file" $optional_libraries
fi
